...
class UserProfileSerializer(ModelSerializer):
    ...
    def validate(self, value):
        ...
        if re.match('^(AB|BM|HB|KH|MP|MC|KB|PP)\d{7}$', passport_number) is None:
            errors[
                'passport_number'] = 'Номер паспорта должен иметь формат (' \
                                     'AB|BM|HB|KH|MP|MC|KB|PP)ЦЦЦЦЦЦЦ.'
        if re.match('^\d{7}[A-Z]{1}\d{3}[A-Z]{2}\d{1}$', identification_number) is None:
            errors[
                'identification_number'] = 'Идентификационный номер должен подходить под формат ' \
                                           'ЦЦЦЦЦЦЦБЦЦЦББЦ.'
