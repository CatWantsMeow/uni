...
from jupiter_auth.api.users.serializers import (
    ChangePasswordSerializer, CreateAdminSerializer, UserSerializer
)
...
class UserView(ModelViewSet):
    ...
    def activate(self, request, *args, **kwargs):
        ...
        try:
            send_mail('no-reply@jupiter.com', user.email, 'Ваш аккаунт подтвержден', message)
    ...
    def deactivate(self, request, *args, **kwargs):
        ...
        try:
            send_mail('no-reply@jupiter.com', user.email, 'Ваш аккаунт отключен', message)
        except Exception as e:
            raise ValidationError('Ошибка при отправке письма: {}'.format(e))
        return Response(status=status.HTTP_200_OK)
