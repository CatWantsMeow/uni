    ...
    def leave_create_claim_validate(value):
        ...
        if template.type == template.TYPE_SELLER:
            if re.match('^301(2|3|4|5)\d{9}$', account_number) is None:
                errors[
                    'account_id'] = 'Номер расчетного счета должен иметь формат 301(2,3,' \
                                    '4)ЦЦЦЦЦЦЦЦЦ ' \
                                    'для коммерческих орг., ИП, физ. лиц. соответственно.'
        else:
            if re.match('^3014\d{9}$', account_number) is None:
                errors[
                    'account_id'] = 'Номер расчетного счета физ. лица должен иметь формат ' \
                                    '3014ЦЦЦЦЦЦЦЦЦ.'
        if not (min_amount <= amount <= max_amount):
            errors['amount'] = 'Допустимая сумма: от {} BYN до {} BYN.'.format(min_amount,
                                                                               max_amount)
        if not (min_duration <= duration <= max_duration):
            errors['duration'] = 'Допустимый срок: от {} до {} месяцев.'.format(min_duration,
                                                                                max_duration)
        if len(errors):
